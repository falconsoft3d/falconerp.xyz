...
export default function PurchaseInvoicesPage() {
  ...
  const fetchInvoices = async (companyId: string) => {
    try {
      const res = await fetch(`/api/invoices?companyId=${companyId}&type=PURCHASE`);
      if (res.ok) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-3xl font-bold text-gray-800">Facturas de Compra</h2>
          <p className="text-gray-600 mt-1">Gestiona las facturas de tus proveedores</p>
        </div>
