...
export default function NewPurchaseInvoicePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setError(data.error || 'Error al crear factura de compra');
      }
    } catch (error) {
      console.error('Error creating purchase invoice:', error);
      setError('Error al crear factura de compra');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        <h2 className="text-3xl font-bold text-gray-800">Nueva Factura de Compra</h2>
        <p className="text-gray-600 mt-1">Registra una factura recibida de un proveedor</p>
      </div>
              ...
              >
                ...
                {suppliers.map((supplier) => (
                  <option key={supplier.id} value={supplier.id}>
                    {supplier.name} {supplier.nif && `- ${supplier.nif}`}
                  </option>
                ))}
              </select>
            </div>
